...
open class CircleProgressView: UIView {
    ...
    var value: String = "" {
        willSet {
            ...
            valueLabel.textColor = progressTintColor
        }
    }
    
    fileprivate let valueLabel = specify(UILabel(), {
    ...
    open override func didMoveToWindow() {
        ...
        if let window = window {
            ...
            progressLayer.timer = { [weak self] timer in
                guard let `self` = self else { return }
                self.value = 0 << (self._duration - timer * self._duration).toString()
            }
            progressLayer.setNeedsDisplay()
        }
    }
    ...
    open func updateProgress(_ progress: CGFloat, animated: Bool = true, initialDelay: CFTimeInterval = 0, duration: CFTimeInterval? = nil, completion: CompletionBlock? = nil) {
        ...
        if animated {
            ...
            value = 0 << (progress * _duration).toString()
...
private extension CircleProgressView {
    ...
    class ProgressLayer: CALayer {
        @NSManaged var timer: ((CGFloat) -> Void)?
